...
tryCatch({
  ...
  results_data <- results_data %>%
    rowwise() %>%
    mutate(
      id_temp = paste0(subject_id, "_", sample_number)
    ) %>%
    ungroup()
  
  # Identify which results to insert vs update - fixing the join issue
  results_with_existing <- results_data %>%
    left_join(existing_results, by = c("subject_id", "sample_number", "sample_id"), suffix = c("", ".y"))
  
  # Now fix the ID and operation assignment
  results_with_existing <- results_with_existing %>%
    mutate(
      id = ifelse(is.na(id.y), paste0(id_temp, "_", substr(UUIDgenerate(), 1, 8)), id.y),
      operation = ifelse(is.na(id.y), "insert", "update")
    ) %>%
    select(-id_temp, -id.y)
  
  results_to_insert <- results_with_existing %>%
    filter(operation == "insert") %>%
    select(-operation)
  
  results_to_update <- results_with_existing %>%
    filter(operation == "update")
  
  cat("\nResults summary:\n")
  cat("Total results:", nrow(results_data), "\n")
  cat("Results to insert:", nrow(results_to_insert), "\n")
  cat("Results to update:", nrow(results_to_update), "\n")
  
  # Insert new results
  if (nrow(results_to_insert) > 0) {
    cat("\nInserting", nrow(results_to_insert), "new results records...\n")
    dbWriteTable(
      con,
      name = Id(schema = "laboratory", table = "omics_results"),
      value = results_to_insert,
      append = TRUE
    )
  }
  
  # For updates, we'll use an approach with temporary tables to handle the complexity
  if (nrow(results_to_update) > 0) {
    cat("\nUpdating", nrow(results_to_update), "existing results records...\n")
    
    # Create a temporary table with the updates
    temp_table_name <- paste0("temp_results_update_", format(Sys.time(), "%Y%m%d%H%M%S"))
    dbWriteTable(
      con,
      name = Id(schema = "public", table = temp_table_name),
      value = results_to_update,
      temporary = TRUE
    )
    
    # Perform the update using the temporary table
    # We exclude operation column and include all other columns
    columns_to_update <- setdiff(names(results_to_update), c("operation", "id"))
    
    # Build SET clause for all columns
    set_clause <- paste(
      paste0(columns_to_update, " = tmp.", columns_to_update), 
      collapse = ", "
    )
    
    # Add updated_at to SET clause
    set_clause <- paste(set_clause, ", updated_at = NOW()")
    
    # Execute update
    update_query <- sprintf(
      "UPDATE laboratory.omics_results r 
       SET %s 
       FROM public.%s tmp 
       WHERE r.id = tmp.id",
      set_clause, 
    ...
    dbExecute(con, sprintf("DROP TABLE public.%s", temp_table_name))
  }
