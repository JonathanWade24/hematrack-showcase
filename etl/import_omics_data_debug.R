...
db_table_structure <- function(schema, table) {
  cat("\n==== DEBUG: Database table structure for", schema, ".", table, "====\n")
...
db_table_structure("laboratory", "omics_subjects")
db_table_structure("laboratory", "omics_results")
...
tryCatch({
  ...
  # Debug: Print raw subjects data structure
  debug_structure(subjects_raw, "subjects_raw")
  ...
  # Debug: Print processed subjects data
  debug_structure(subjects_data, "subjects_data")
  ...
  # Debug: Print existing subjects
  debug_structure(existing_subjects, "existing_subjects")
  ...
  # Debug: Print subjects to insert and update
  debug_structure(subjects_to_insert, "subjects_to_insert")
  debug_structure(subjects_to_update, "subjects_to_update")
  ...
  debug_structure(results_raw, "results_raw") 
  ...
  # Debug: Print processed results data
  debug_structure(results_data, "processed_results_data")
  ...
  # Debug: Print final processed results data
  debug_structure(results_data, "final_results_data")
  ...
  # Debug: Print existing results data
  debug_structure(existing_results, "existing_results")
  ...
  if(nrow(potential_duplicates) > 0) {
    ...
    print(head(potential_duplicates[, c("subject_id", "sample_number", "id", "sample_id.x", "sample_id.y")]))
  }
  ...
  # Use suffix .existing for clarity on columns from the database
  results_with_ids <- results_data %>%
    left_join(existing_results, by = c("subject_id", "sample_number"), suffix = c("", ".existing"))
  
  # Debug: Print joined results data
  debug_structure(results_with_ids, "results_with_ids (joined)")
  
  # Split into new records (need insert) and existing records (need update)
  results_new <- results_with_ids %>%
    filter(is.na(id)) %>%
    select(-id, -sample_id.existing) # Remove all database columns that would cause conflicts
  
  # Debug: Print new results data
  debug_structure(results_new, "results_new (to insert)")
  ...
  if(nrow(results_new) > 0) {
    ...
    # Debug: Print final new results with IDs
    debug_structure(results_new, "results_new (with IDs)")
  ...
  results_existing <- results_with_ids %>%
    filter(!is.na(id)) %>%
    # Ensure we're using the right columns - sample_id from our data, not from the DB
    select(-sample_id.existing) %>%
    mutate(updated_at = Sys.time())
  
  # Debug: Print existing results data to update
  debug_structure(results_existing, "results_existing (to update)")
  ...
  if (nrow(results_new) > 0) {
    ...
    # Print the structure of the data we're about to insert for debugging
    ...
    cat("\nTarget table columns:", paste(target_cols$column_name, collapse=", "), "\n")
    cat("Data to insert columns:", paste(names(results_new), collapse=", "), "\n")
    
    # Ensure we only have columns that exist in the target table
    valid_cols <- intersect(names(results_new), target_cols$column_name)
    missing_cols <- setdiff(target_cols$column_name, names(results_new))
    extra_cols <- setdiff(names(results_new), target_cols$column_name)
    
    cat("Valid columns:", paste(valid_cols, collapse=", "), "\n")
    if(length(missing_cols) > 0) {
      cat("Missing columns (will use defaults):", paste(missing_cols, collapse=", "), "\n")
    }
    if(length(extra_cols) > 0) {
      cat("Extra columns (will be dropped):", paste(extra_cols, collapse=", "), "\n")
      # Remove extra columns that aren't in the target table
      results_new <- results_new %>% select(all_of(valid_cols))
    }
    
    # Debug: Print final data structure that will be inserted
    debug_structure(results_new, "final_data_to_insert")
  ...
  if (nrow(results_existing) > 0) {
    ...
    for (i in 1:nrow(results_existing)) {
      ...
      # Debug: Print update query for first few records
      if (i <= 3) {
        cat("Sample update query (record", i, "):\n")
        cat(sprintf("UPDATE laboratory.omics_results SET %s WHERE id = '%s'\n", set_clause, id_val))
      }
