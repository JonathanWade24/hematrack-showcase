...
-- Omics subjects table (stays in clinical as it links patients to samples)
CREATE TABLE clinical.omics_subjects (
    subject_id VARCHAR(20) PRIMARY KEY,
    patient_mrn VARCHAR(50) NOT NULL REFERENCES phi.patients(patient_mrn),
    project VARCHAR(20) DEFAULT 'OMI' NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    
    -- Ensure unique MRN per project
    UNIQUE (patient_mrn, project)
);

-- Omics results table
CREATE TABLE clinical.omics_results (
...
-- Create updated_at trigger for omics_subjects
CREATE TRIGGER update_omics_subjects_timestamp
    BEFORE UPDATE ON clinical.omics_subjects
    FOR EACH ROW
    EXECUTE FUNCTION phi.update_timestamp();

-- Add audit trigger for omics_subjects
CREATE TRIGGER audit_omics_subjects_changes
    AFTER INSERT OR UPDATE OR DELETE ON clinical.omics_subjects
    FOR EACH ROW EXECUTE FUNCTION audit.audit_trigger();

-- Create index for omics_subjects
CREATE INDEX idx_omics_subjects_mrn ON clinical.omics_subjects(patient_mrn);

-- Create updated_at triggers
CREATE TRIGGER update_omics_results_timestamp
    BEFORE UPDATE ON clinical.omics_results
    FOR EACH ROW
    EXECUTE FUNCTION phi.update_timestamp();

-- Add audit triggers
CREATE TRIGGER audit_omics_results_changes
    AFTER INSERT OR UPDATE OR DELETE ON clinical.omics_results
    FOR EACH ROW EXECUTE FUNCTION audit.audit_trigger();

-- Create indexes
CREATE INDEX idx_omics_results_subject_id ON clinical.omics_results(subject_id);
CREATE INDEX idx_omics_results_collection_date ON clinical.omics_results(date_of_collection);
CREATE INDEX idx_omics_results_sample_number ON clinical.omics_results(subject_id, sample_number);
